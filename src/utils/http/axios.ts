...
axios.interceptors.response.use(
  ...
  error => {
    if (error.response?.status === 401 && !WHITE_URL_LIST.includes(error.config.url)) {
      // const userStore = useUserStore();
      // userStore.logout();
      router.push({
        name: 'Login'
...
const upload = (url: string, data: any) => axios({
  url,
  data,
  method: 'post',
...
// 下载
const download = (url: string, responseType: ResponseType = 'arraybuffer') => axios({
  url,
  method: 'GET',
  responseType,
  headers: {
...
// post传参形式的下载
const downloadWidthParam = (url: string, data: any, responseType: ResponseType = 'blob') => axios({
  url,
  data,
  method: 'POST',
  responseType,
